<?php
class Daftar_design extends CI_Controller {
    ...
    <div class="form-group">
        ...
        foreach($ctgry as $c){
        if ($g['design_category']==$c['category_nama']) {
            $html.='<option value="'.$c['category_nama'].'" selected>'.$c['category_nama'].'</option>';
        }else{
            $html.='<option value="'.$c['category_nama'].'">'.$c['category_nama'].'</option>';
        }
        }
        $html.='</select>
    </div>
    <button id="'.$g['design_id'].'" type="button" class="btn btn-primary update">Save changes</button>
        ...
        <br>
        <a style="width: 50%;" target="_black" href="'.base_url('Editor?design=').$g['design_id'].'&level=1" class="btn btn-primary">Edit Design</a></center>
        </div>
    </div>';
    echo $html;
    }
    ...
    <button id="'.$g['design_id'].'" type="button" class="btn btn-primary update">Save changes</button>
        ...
        <p>'.$g['design_width'].' X '.$g['design_height'].'</p>
        <br>
        <a style="width: 50%;" target="_black" href="'.base_url('Editor?design=').$g['design_id'].'&level=cp" class="btn btn-primary">Edit Design</a></center>
        </div>
    </div>';
    echo $html;
    }
